...
## M1 — Server core
- [x] ⚠ **1.1 RailKit client** *(done except checkPNRStatus fixture — pending a real PNR)* [PRD §6]: typed wrapper per upstream endpoint; input validation (5-digit train, 10-digit PNR, date conversion YYYY-MM-DD ↔ upstream DD-MM-YYYY); `RAILKIT_API_KEY` from env only; fixtures in `src/railkit/__fixtures__/` (use the recorded sample payloads in docs/fixtures/); `trainHistory` 404 → `NOT_YET_AVAILABLE`.
